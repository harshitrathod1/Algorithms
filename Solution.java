...
class Solution {
    public static int maxFrequency(int[] nums, int k) {
        ...
        int lastElement = nums[i];

        while (i < nums.length && j < nums.length){
            int diff = nums[j] - lastElement;
            if(diff * (j - i) <= operationsLeft){
                maxFreq = Math.max(maxFreq,(j - i + 1));
                operationsLeft = operationsLeft - ((j - i) * diff);
                lastElement = nums[j];
                j++;
            }else{
                operationsLeft = operationsLeft + (lastElement - nums[i]);
                lastElement = nums[j];
                i++;
            }
        }
